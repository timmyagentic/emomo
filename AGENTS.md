...
- `cd backend && GOTOOLCHAIN=go1.26.2 go run github.com/bufbuild/buf/cmd/buf@v1.69.0 generate` — regenerate Go protobuf code after schema-level type changes.
...
- Backend schema-level types: define enums/structured values in `backend/proto/emomo/v1/schema.proto`; generated Go code lives under `backend/internal/idl/`. This protobuf value schema is intentionally limited to column-level values — not a wire / RPC contract.
