...
API 使用当前后端 schema：关系库核心表为 `memes`、`meme_annotations`、`meme_vectors`；默认检索链路直接使用图片向量，VLM/OCR 只作为辅助 annotation。schema 级类型由 `backend/proto/emomo/v1/schema.proto` 定义。
