...
// vectorTypeQueryArg converts a protobuf VectorType into a plain int32 suitable
// for use as a GORM Where(...) bind parameter against the integer
// `meme_vectors.vector_type` column.
//
// Why this is necessary: pb.VectorType is a typed int32 that also implements
// fmt.Stringer (generated by protoc-gen-go). When such a value is passed into a
// Where(...) clause it bypasses the GORM `protoenum` serializer (which only
// runs on struct field writes via Create/Save) and reaches the pgx driver
// directly. pgx then prefers the Stringer form ("VECTOR_TYPE_IMAGE"), which
// Postgres rejects with `invalid input syntax for type integer`. Casting to
// plain int32 strips the named type so the driver encodes the value as INT4.
func vectorTypeQueryArg(vt pb.VectorType) int32 {
	return int32(persistence.NormalizeVectorType(vt))
}
...
func (r *MemeVectorRepository) ExistsByMemeIDCollectionAndVectorType(ctx context.Context, memeID, collection string, vectorType pb.VectorType) (bool, error) {
	var count int64
	if err := r.db.WithContext(ctx).Model(&domain.MemeVector{}).
		Where("meme_id = ? AND collection = ? AND vector_type = ?", memeID, collection, vectorTypeQueryArg(vectorType)).
		Count(&count).Error; err != nil {
		return false, err
	}
	return count > 0, nil
