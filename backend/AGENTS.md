...
- Schema: keep core relational data centered on `memes`, `meme_annotations`, `meme_vectors`, and the provenance-only `meme_metadata` (never read by search). Protobuf owns API DTOs, generated frontend/backend DTOs, closed cross-boundary enums, and only the explicit structured JSON columns `memes.image_info` / `meme_annotations.labels`. The GORM models plus `internal/repository/db.go` own relational tables, columns, indexes, and migrations. Do not move open business/config values (`category`, `tags`, `collection`, embedding/analyzer model names, source IDs) into protobuf enums. Re-run `buf generate` after editing `.proto` so `gen/` stays in sync.
